...
void PBOSample::Init() {
    ...
    //初始化 PBO
    glGenBuffers(2, uploadPboIds);
    int imgByteSize = m_RenderImage.width * m_RenderImage.height * 4;//RGBA
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, uploadPboIds[0]);
    glBufferData(GL_PIXEL_UNPACK_BUFFER, imgByteSize, 0, GL_STREAM_DRAW);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, uploadPboIds[1]);
    glBufferData(GL_PIXEL_UNPACK_BUFFER, imgByteSize, 0, GL_STREAM_DRAW);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);

    glGenBuffers(2, downloadPboIds);
    glBindBuffer(GL_PIXEL_PACK_BUFFER, downloadPboIds[0]);
    glBufferData(GL_PIXEL_PACK_BUFFER, imgByteSize, 0, GL_STREAM_READ);
    glBindBuffer(GL_PIXEL_PACK_BUFFER, downloadPboIds[1]);
    glBufferData(GL_PIXEL_PACK_BUFFER, imgByteSize, 0, GL_STREAM_READ);
...
void PBOSample::Destroy() {
    ...
    if (downloadPboIds[0]) {
        glDeleteBuffers(2, downloadPboIds);
    }

    if (uploadPboIds[0]) {
        glDeleteBuffers(2, uploadPboIds);
    }
...
void PBOSample::DownloadPixels() {
    ...
    BEGIN_TIME("DownloadPixels glReadPixels with PBO")
        glBindBuffer(GL_PIXEL_PACK_BUFFER, downloadPboIds[index]);
...
//    BEGIN_TIME("DownloadPixels PBO glMapBufferRange")
        glBindBuffer(GL_PIXEL_PACK_BUFFER, downloadPboIds[nextIndex]);
